...
function rangeWithAnchors(
  ...
  // ensure time order
  const e = Math.min(entryMs, exitMs);
  const x = Math.max(entryMs, exitMs);

  const duration = x - e;
  const span = duration / (exitPct - entryPct); // total visible span
  const from = e - entryPct * span;
  const to = from + span;

  return { from, to };
}

const CHART1M_LAST_SYMBOL_KEY = 'aixsignal:chart1m:last-symbol';

const Binance1mChartContainer: React.FC = () => {
  ...
  const [symbol, setSymbol] = useState<string>(
    () => symbolFromQuery || readLastChartSymbol(CHART1M_LAST_SYMBOL_KEY) || defaultSymbol
  );
  ...
  useEffect(() => {
    if (activeSymbol) writeLastChartSymbol(CHART1M_LAST_SYMBOL_KEY, activeSymbol);
  }, [activeSymbol]);

  // 모의매매 포지션 클릭 시, 진입 당시 스트림이 Wave(10분봉)면 해당 차트로 이동시킨다.
  ...
  const [pendingSignal, setPendingSignal] = useState<{ event: any; entryEvent?: any | null } | null>(null);
  ...
  const parseSnapshotMs = (value: string | null) => {
    ...
    if (/^\d+(\.\d+)?$/.test(trimmed)) {
      ...
      if (num < 2_000_000_000_0) return Math.round(num * 1000);
      if (num >= 10_000_000_000_000) return Math.round(num / 1000);
  ...
  const fmtYYYYMMDD = (iso: string | null) => (iso ? iso.slice(0, 10) : "");
  ...
  const snapshotRange = useMemo(() => {
    ...
    if (Number.isFinite(exitMs)) {
      ...
      const padding = Math.max(30 * 60 * 1000, duration * 0.2);
      return {
  ...
  const exitSignals = useMemo(() => {
    return (signalEvents ?? [])
      ...
      .filter((ev: any) => String(ev.signal_type).toLowerCase() === "exit")
       
      .sort((a: any, b: any) => Number(b.timestamp_ms) - Number(a.timestamp_ms))
      .slice(0, 8);
  }, [signalEvents]);
  ...
  const handleConfirmDownload = async () => {
    ...
    try {
      ...
      if (!cycle?.id) {
        alert("Signal cycle not found for this exit.");
        return;
      }
      ...
      const item: SnapshotItem = {
        ...
        exitTimeISO: exitISO,
        date: fmtYYYYMMDD(entryISO),
        views: 0,
        likes: 0,
        period: 0,
        barinterval: cycle.barinterval || event.bar_interval || "5m",
        durationSec,
      };
      console.warn("Snapshot item:", item);

      await handleDownload(item);
    } catch (e) {
      console.error(e);
      alert(`Snapshot failed: ${String((e as Error)?.message || e)}`);
    } finally {
  ...
  useEffect(() => {
    ...
    if (!exitTimeParam) {
      ...
      applyRange(entryMs - twentyMinutes, entryMs + twentyMinutes);
      return;
    }
    ...
    const { from, to } = rangeWithAnchors(entryMs, exitMs, 0.2, 0.8);
    applyRange(from, to);
  }, [searchParams, initialLoading, snapshotMode, setVisibleTimeRange, gotoEnd]);
  ...
  useEffect(() => {
    ...
    setVisibleTimeRange(snapshotRange.startMs, snapshotRange.endMs);
  }, [snapshotMode, snapshotRange, initialLoading, activeTf, handleTimeframeClick, setVisibleTimeRange]);
  ...
  useEffect(() => {
    ...
    const exitPriceParam = searchParams.get("exitPrice");

    const entryMs = parseSnapshotMs(entryTimeParam);
    const exitMs = parseSnapshotMs(exitTimeParam);
    const entryPrice = entryPriceParam ? Number(entryPriceParam) : NaN;
    const exitPrice = exitPriceParam ? Number(exitPriceParam) : NaN;
    ...
    if (
      ...
      setMockTradeOverlay({
        fills: [
          { time: Math.floor(entryMs / 1000) as UTCTimestamp, price: entryPrice, fillType: "entry" },
          { time: Math.floor(exitMs / 1000) as UTCTimestamp, price: exitPrice, fillType: "exit" },
        ],
      });
    } else {
      clearMockTradeOverlay();
    }
  }, [searchParams, initialLoading, snapshotMode, setMockTradeOverlay, clearMockTradeOverlay]);
  ...
  async function uploadAndCreateShareLink(blob: Blob, symbol: string, barInterval: string) {
    ...
    const { error: insErr } = await supabase.from("chart_snapshots").insert({
      slug,
      image_path: imagePath, // ✅ matches your table column
      symbol,
  ...
  // ✅ NEW: Full DOM snapshot capturing overlays + volume + colors
  ...
  async function takeFullSnapshotBlob(): Promise<Blob> {
    ...
    const overlayDivs: (HTMLElement | null)[] = [
      ...
      signalSvgOverlayRef.current as HTMLElement | null,
    ];
  ...
  const waitForSnapshotRender = async () => {
    const root = snapshotRef.current;
    if (!root) return;
  ...
  return (
    <>
      <div key={symbol} className={wrapperClassName}>{renderChartView()}</div>
      <Dialog
        open={confirmOpen}
        onOpenChange={(open) => {
          setConfirmOpen(open);
...
export default Binance1mChartContainer;
