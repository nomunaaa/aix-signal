...
import { longBgColorFromValue, shortCandleColorFromValue } from "@/chart/chartConfig";
...
export function applyMarkers(
    ...
    const snapToCandleTime = (timestampLike: number | string): UTCTimestamp | null => {
        if (timestampLike === null) return null;

        let tsMs = typeof timestampLike === "string" ? Number(timestampLike) : timestampLike;
...
export function updateBackgroundBandsDOM(
    ...
    const lastCandleMs = Number(candles[candles.length - 1].time) * 1000 || Date.now();

    overlayEl.innerHTML = "";

    const signals = [...trendEvents]
        .filter((s) => s.type === "trend_long" && (s.screen_direction ?? s.value) !== undefined)
        .sort((a, b) => new Date(a.ts).getTime() - new Date(b.ts).getTime());
    ...
    for (let i = 0; i < signals.length; i++) {
        ...
        band.style.backgroundColor = longBgColorFromValue(sig.screen_direction ?? sig.value!);
