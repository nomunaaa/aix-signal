...
import { HistoryEntryCountLink } from '@/components/proof/HistoryEntryCountLink';
...
function MetricCells({ slice, selection, id, period, language }: { slice: ProofPageMock['totalStats'][0]['total']; selection?: ProofStatsSelection; id: SignalStreamOptionId; period: '30d' | '90d' | 'all'; language: 'ko' | 'en' | 'ja' }) {
  ...
  return <>
    <td className="border-l border-border/60 px-1 py-3 text-right"><HistoryEntryCountLink symbol="" count={slice.cycleCount} period={period} asOfIso={slice.asOfIso} streams={streams} trendModes={trends} tradingCategories={['E2X2']} signalOptions={[id]} /></td>
...
export function SelectionPageView() {
  ...
  const stats = useMemo(() => new Map(OPTIONS.map((option) => {
    if (!option.selection) return [option.id, null] as const;
    ...
    const symbols = rows
      ...
      .map((row) => row.symbol)
      .filter((symbol) => !showFavoritesOnly || favorites.has(symbol.trim().toUpperCase()));
    return [option.id, reconstructTotalStatsForSymbols(data.buckets, symbols, [option.selection.stream], [option.selection.trendMode], ['E2X2'], [option.selection])[0]] as const;
  })), [
    data.buckets,
    favorites,
    qualityPeriod,
  ...
  return <div className="mx-auto min-h-screen w-full max-w-[1400px] bg-background px-4 py-8 pb-20 text-foreground md:px-5">
    <div className="sticky z-40 -mx-4 mb-8 border-b border-border/40 bg-background/95 px-4 py-2 backdrop-blur md:-mx-5 md:px-5" style={{ top: 'var(--header-height)' }}>
      <div className="mx-auto flex max-w-[1400px] flex-wrap items-center gap-3">
        <FavoriteScopeControls symbols={allSymbols} /><StreamSelector />
        <SymbolQualityFilter copy={copy} period={qualityPeriod} onPeriodChange={setQualityPeriod} />
      </div>
    </div>
    <div className="overflow-x-auto rounded-lg border border-border bg-card">
      <table className="w-full min-w-[1120px] table-fixed border-collapse text-[10px] xl:text-xs">
        <colgroup><col className="w-[8%]" /><col className="w-[5%]" />{Array.from({ length: 18 }).map((_, index) => <col key={index} className="w-[4.83%]" />)}</colgroup>
        <thead className="bg-muted text-muted-foreground"><tr><th rowSpan={2} className="px-2 py-2">Group</th><th rowSpan={2} className="px-2 py-2">Trend</th><th colSpan={6} className="border-l border-border/60 px-1 py-2">Last 30 days</th><th colSpan={6} className="border-l border-border/60 px-1 py-2">Last 3 months</th><th colSpan={6} className="border-l border-border/60 px-1 py-2">All time</th></tr><tr>{['Entries','Account Return','Account Profit','Win Rate','Ratio','Hold Time'].map((label) => <Fragment key={label}><th className="border-l border-border/60 px-1 py-2">{label}</th></Fragment>)}{['Entries','Account Return','Account Profit','Win Rate','Ratio','Hold Time'].map((label) => <th key={`3-${label}`} className="border-l border-border/60 px-1 py-2">{label}</th>)}{['Entries','Account Return','Account Profit','Win Rate','Ratio','Hold Time'].map((label) => <th key={`a-${label}`} className="border-l border-border/60 px-1 py-2">{label}</th>)}</tr></thead>
        <tbody className="divide-y divide-border/60">{OPTIONS.map((option, index) => { const row = stats.get(option.id); const selected = optionFilter[option.id]; return <tr key={option.id} className={cn('transition-colors', selected && 'bg-yellow-100/80 dark:bg-yellow-500/15')}><td className="px-2 py-3 font-semibold">{index % 3 === 0 ? option.group : ''}</td><td className="px-2 py-3"><span className={cn('inline-flex rounded px-1.5 py-0.5 text-[10px] font-bold', SIGNAL_OPTION_BADGE_CLASS[signalOptionTone(option.id)])}>{option.id}</span></td><MetricCells slice={row?.recent30 ?? emptyStatsSlice()} selection={option.selection} id={option.id} period="30d" language={language} /><MetricCells slice={row?.recent3mo ?? emptyStatsSlice()} selection={option.selection} id={option.id} period="90d" language={language} /><MetricCells slice={row?.total ?? emptyStatsSlice()} selection={option.selection} id={option.id} period="all" language={language} /></tr>; })}</tbody>
      </table>
    </div>
  </div>;
}
