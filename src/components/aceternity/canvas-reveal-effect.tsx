...
function buildShaderMaterial(
  source: string,
  uniformsConfig: Uniforms,
  width: number,
  height: number
): THREE.ShaderMaterial {
  const preparedUniforms: Record<string, THREE.IUniform> = {};

  for (const uniformName in uniformsConfig) {
    const uniform = uniformsConfig[uniformName];
...
/** @react-three/fiber 없이 WebGL만 사용 — Next + React 19에서 reconciler 내부 접근 오류 방지 */
const WebGLShaderPlane = ({
  source,
  uniforms,
  maxFps = 60,
}: {
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      if (!renderer) {
        ...
        material = buildShaderMaterial(source, uniforms, w, h);
